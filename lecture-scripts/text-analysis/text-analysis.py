...
def spoken_line_counts(lines):
    line_counts = {}
    for line in lines: 
        if line in characters:
            current_char = line
            if current_char not in line_counts:
                line_counts[current_char] = 0
        else:
            line_counts[current_char] += 1
...
line_counts = spoken_line_counts(lines)
print(line_counts)
