...
In the last several lectures, we've learned about lists and worked several examples. Lists are an extremely flexible and useful *data structure*. A data structure is just a *structure* for holding *data*. Different data structures have different purposes. Lists are really good for when you want to organize a bunch of data in an *ordered relationship*: one element comes after the other.

There are LOTS of other data structures, all of which are suited for representing different kinds of relationships between data. 

In this lecture, we'll introduce a few more data structures. We'll focus on the most useful of these -- dictionaries. If you continue in computer science, you'll take an entire class on data structures in CSCI 0201. 
"""
...
A tuple is similar to a list, an ordered sequence of data. Unlike lists, tuples are *immutable*: once created, a tuple can't be changed. For example, you can't .append() anything to a tuple. 
...
Unlike tuples, dictionaries are *extremely useful* for a wide range of programming tasks. A dictionary is a data structure that expresses a relationship between pairs of pieces of data. For example, suppose we want to represent the relationship between movie characters and their movies: 
...
A dictionary is a data structure that can express this kind of relationship. Syntactically, a dictionary is denoted by curly braces {}. It contains pairs of data. Each pair is separated by a comma, and the two elements of the pair are separated by a colon :. For readability, it's common to write dictionaries with one pair on each line: 
"""
...
Remember the Fibonacci numbers? Our full recursive implementation looked like this: 
"""
...
"""
This is a totally fine definition, but as we saw in guided discovery, just computing fib(40) or so can take a noticeably long time on your computer. The reason is that we actually have to compute each Fibonacci number a large number of times, leading to a total of approximately 2^n computations. But this wouldn't be necessary if we could remember which ones we had already computed, and just use them: 
"""
...
A set is a collection of values that is *unordered*. Furthermore, all elements of a set are unique: there can be no repetitions. 
...
"""
Like lists, sets have a len(), which is just the number of items in a set. Lists can also be converted into sets like this: 
"""
