...
number or a string. A very classical example of a reducing operation is to add up a list of numbers, producing a single number (the total). 

For a related example, let's compute the average of a list: 
"""

def mean(L):
    total = 0 # why not sum = 0?
    for entry in L:
        total = total + entry 
    num_entries = len(L)        
